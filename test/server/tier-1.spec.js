    ...
    describe('Basic Fields: name and userType', () => {
      describe('name', () => {
        ...
        it('name must be unique', async () => {
          ...
          try {
            ...
            expect(ex).to.not.equal(NOO);
            const error = ex.errors.find(e => e.path === 'name');
        ...
        it('name cannot be an empty string', async () => {
          ...
          try {
            ...
            throw NOO; //we should not get here
          } catch (ex) {
            expect(ex).to.not.equal(NOO);
            const error = ex.errors.find(e => e.path === 'name');
            expect(error).to.be.ok;
      ...
      describe('userType', () => {
        ...
        it('userType cannot be null', async () => {
          try {
            const aliPromise = await User.create({
              name: 'ALI',
              userType: null,
            });
            throw NOO; //should not get here
          } catch (ex) {
            expect(ex).to.not.equal(NOO);
            const error = ex.errors.find(e => e.path === 'userType');
            expect(error).to.be.ok;
          }
        });
        ...
        it('userType can ONLY be either "STUDENT" or "TEACHER"', async () => {
          try {
            const aliPromise = await User.create({
              name: 'ALI',
              userType: 'EAGER_TO_LEARN',
            });
            throw NOO; //show not get here
          } catch (ex) {
            expect(ex).to.not.equal(NOO);
          }
        });
