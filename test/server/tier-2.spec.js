...
describe('Tier 2: Eager Loading, One-To-Many Associations', () => {
  beforeEach(async () => {
    ...
    wanda.mentorId = lucy.id;
    await Promise.all([moe.save(), wanda.save()]);
  });
  ...
  describe('Express', () => {
    describe('GET /api/users/teachers', () => {
      ...
      it('responds with all teachers and their mentees', async () => {
        ...
        const _moe = lucy.mentees.find(m => m.name === 'MOE');
        expect(_moe.name).to.equal('MOE');
        const _wanda = lucy.mentees.find(m => m.name === 'WANDA');
        expect(_wanda.name).to.equal('WANDA');
      });
    });
  });
});
